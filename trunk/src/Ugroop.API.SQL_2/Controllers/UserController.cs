...
namespace Ugroop.API.SQL.Controllers {
    ...
    public class UserController : SecurityController {
        ...
        // POST : Get_AccountTravelDocumentByDocumentId
        [HttpPost]
        public HttpResponseMessage Get_AccountTravelDocumentByDocumentId(JObject jsonData) {
            int id = Convert.ToInt32(JsonData.Instance(jsonData).Get_JsonObject("id"));
            var data = AccountService.Get_TravelDocumentByDocumentId(id);
            return new HttpResponseMessage {
                Content = new StringContent(data.JsonSerialize())
            };
        }
