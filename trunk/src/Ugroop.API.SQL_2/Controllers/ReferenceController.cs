...
namespace Ugroop.API.SQL.Controllers {
    public class ReferenceController : SecurityController {
        ...
        public HttpResponseMessage Add_TimeZone(JObject jsonData) {
            var timezoneData = JEntity<UGroopData.Sql.Repository2.Data.TimeZone>.Instance().Get_Entity(jsonData);
        ...
        [HttpPost]
        public HttpResponseMessage Get_AllTourActivityType(JObject jsonData) {
            return new HttpResponseMessage {
                Content = new StringContent(ReferenceService.Get_AllTourActivityType().JsonSerialize())
            };
        }


        [HttpPost]
        public HttpResponseMessage Get_TourActivityTypeById(JObject jsonData) {
            int id = Convert.ToInt32(JsonData.Instance(jsonData).Get_JsonObject("id"));
            return new HttpResponseMessage {
                Content = new StringContent(ReferenceService.Get_TourActivityTypeById(id).JsonSerialize())
            };
        }
